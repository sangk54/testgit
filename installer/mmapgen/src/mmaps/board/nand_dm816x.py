...
# RidgeRun, LLC.
#
# Memory map calculations for nand installation mode, DM816x board.
#
# ==========================================================================
...
class NandInfoDm816x():
    nand_blk_size = None
    nand_page_size = None
    mtdparts = None

class NandMemoryMapDm816x(MemoryMap):
    ...
    def _check_imgs(self):
        for img in self.imgs.items():
            ...
            if component is 'fs':
                if self._fs_name() == NandMemoryMapDm816x.FS_NFS:
                    continue
    ...
    def generate_mmap(self):
        ...
        if (self._fs_name() != NandMemoryMapDm816x.FS_NFS and
            self._fs_name() != NandMemoryMapDm816x.FS_SD):
            self._generate_fs()
    ...
    def _generate_bootloader_env(self):
		if self._c.has_option('CONFIG_UBOOT_FW_PRINTENV'):
			...
			uboot_env.start_blk = 19 # Default DM816x env start blk (0x260000)
			uboot_env.size_blks = 1 # Default DM816x env size (0x2000)
    ...
    def _fs_name(self):
        ...
        if nfs and nfs == 'y': return NandMemoryMapDm816x.FS_NFS
        elif sd and sd == 'y': return NandMemoryMapDm816x.FS_SD
        elif initrd and initrd == 'y': return NandMemoryMapDm816x.FS_INITRD
        elif jffs2 and jffs2 == 'y': return NandMemoryMapDm816x.FS_JFFS2
        elif ubifs and ubifs == 'y': return NandMemoryMapDm816x.FS_UBIFS
        elif cramfs and cramfs == 'y': return NandMemoryMapDm816x.FS_CRAMFS
        elif romfs and romfs == 'y': return NandMemoryMapDm816x.FS_ROMFS
        else: return ''
