...
# RidgeRun, LLC.
#
# Memory map calculations for nand installation mode, IMX6 board.
#
# ==========================================================================
...
class NandMemoryMapImx6(MemoryMap):
    ...
    def validate_config(self):
        if (not self._c.has_option('CONFIG_INSTALLER_MODE_ATTACHED_BOARD')):
            raise MemoryMapException('You are asking to generate the NAND ' 
                'memory map, but  CONFIG_INSTALLER_MODE_ATTACHED_BOARD '
                'in not set.')
    ...
    def _check_imgs(self):
        for img in self.imgs.items():
            ...
            if component is 'fs':
                if self._fs_name() == NandMemoryMapImx6.FS_NFS:
                    continue
    ...
    def generate_mmap(self):
        ...
        if (self._fs_name() != NandMemoryMapImx6.FS_NFS and
            self._fs_name() != NandMemoryMapImx6.FS_SD):
            self._generate_fs()
            
    def _generate_spl(self):
        start_blk = 0
        size_blks = 16
        img = '%s/%s' % (self._devdir, self.imgs['ipl'])
        size = os.path.getsize(img)
        size_blks = self._bytes_to_blks(size)
        spl = NandPartition(self.names['ipl'])
        spl.image = None
        spl.start_blk = start_blk
        spl.size_blks = size_blks
        self._partitions.append(spl)
    ...
    def _generate_bootloader(self):
        ...
        for part in self._partitions: 
            if part.name == self.names['ipl']:
                spl_last_blk = part.start_blk + part.size_blks
        if start_blk < spl_last_blk:
            if self._c.has_option('CONFIG_BSP_ARCH_INSTALLER_UBOOT_FLASH_BLK_START'):
    ...
    def _fs_name(self):
        ...
        if nfs and nfs == 'y': return NandMemoryMapImx6.FS_NFS
        elif sd and sd == 'y': return NandMemoryMapImx6.FS_SD
        elif initrd and initrd == 'y': return NandMemoryMapImx6.FS_INITRD
        elif jffs2 and jffs2 == 'y': return NandMemoryMapImx6.FS_JFFS2
        elif ubifs and ubifs == 'y': return NandMemoryMapImx6.FS_UBIFS
        elif cramfs and cramfs == 'y': return NandMemoryMapImx6.FS_CRAMFS
        elif romfs and romfs == 'y': return NandMemoryMapImx6.FS_ROMFS
        else: return ''
