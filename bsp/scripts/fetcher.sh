    ...
    if [ ! -f $DIR/$PKG.sha1 ] ; then
        ...
        if [ -d $DIR ] && [ -w $DIR ] ; then
            if [ -n "$PKG_SHA1SUM" ] ; then
                ...
                curl -k -f -L $url/$PKG.sha1 > $DIR/$PKG.sha1 || handle_curl_error $? $PKG sha1sum $DIR/$PKG.sha1  $url/$PKG.sha1
            fi
        fi
    fi
}
    ...
    if [ ! -f $DIR/$PKG ] ; then
        case $PKG_TYPE in
            http|ftp|https )
                ...
                curl -k -f -L --limit-rate $FETCHER_MAX_DOWNLOAD_RATE  $URL/$PKG > $DIR/$PKG || { handle_curl_error $? $PKG package $DIR/$PKG $URL/$PKG ; rm -rf $DIR/$PKG ; return 255; };
                ;;
            git )
                ...
                if [ -n "$PKG_REVISION" ] ; then
					if [ -n "$PKG_BRANCH" ] ; then
						git checkout -b $PKG_BRANCH $PKG_REVISION || { echo "Failed to branch to the specified package revision: $PKG_REVISION" ; exit 255; }
					else
						git checkout -b stable $PKG_REVISION || { echo "Failed to branch to the specified package revision: $PKG_REVISION" ; exit 255; }
                    fi
                else
					git checkout $PKG_BRANCH || { echo "Failed to branch to the specified package revision: $PKG_REVISION" ; exit 255; }
                fi
                #Create the package tarball
                if [ -n "$PKG_NO_EXCLUDE_DOT_GIT" ] ; then
					tar $VERBOSE_ARGS -cjf $DIR/$PKG . || { echo "Failed to create archive: $DIR/$PKG" ; exit 255; }
                else
...
function process_package {
    ...
    case $PKG_TARBALL in
        *.tar.gz | *.tgz)
            tar $VERBOSE_ARGS $TAR_ARGS -xzf $DOWNLOADPATH/$PKG_TARBALL $TAR_SUBDIR
            ;;
        *.tar.bz2 )
            tar $VERBOSE_ARGS $TAR_ARGS -xjf $DOWNLOADPATH/$PKG_TARBALL $TAR_SUBDIR
            ;;
        *.tar.xz )
        ...
        *.zip )
            ...
            ;;
        *.tar )
            tar $VERBOSE_ARGS $TAR_ARGS -xf $DOWNLOADPATH/$PKG_TARBALL $TAR_SUBDIR
            ;;
        *.bin )
